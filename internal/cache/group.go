...
func (m *Manager) CreateGroup(ctx context.Context, group *Group) error {
	result, err := m.db.ExecContext(ctx, `
		INSERT INTO cache_groups (name, description) VALUES (?, ?)
...
func (m *Manager) UpdateGroup(ctx context.Context, group *Group) error {
	_, err := m.db.ExecContext(ctx, `
		UPDATE cache_groups SET name = ?, description = ? WHERE id = ?
...
func (m *Manager) DeleteGroup(ctx context.Context, id int64) error {
	result, err := m.db.ExecContext(ctx, `DELETE FROM cache_groups WHERE id = ?`, id)
	if err != nil {
...
func (m *Manager) GetGroup(ctx context.Context, id int64) (*Group, error) {
	...
	err := m.db.QueryRowContext(ctx, `
		SELECT id, name, description FROM cache_groups WHERE id = ?
...
func (m *Manager) GetGroupByName(ctx context.Context, name string) (*Group, error) {
	group := &Group{}
	err := m.db.QueryRowContext(ctx, `
		SELECT id, name, description FROM cache_groups WHERE name = ?
...
func (m *Manager) ListGroups(ctx context.Context) ([]Group, error) {
	rows, err := m.db.QueryContext(ctx, `SELECT id, name, description FROM cache_groups ORDER BY name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (m *Manager) AddConnectionToGroup(ctx context.Context, connID, groupID int64) error {
	_, err := m.db.ExecContext(ctx, `
		INSERT OR IGNORE INTO cache_group_members (cache_id, group_id) VALUES (?, ?)
	`, connID, groupID)
	return err
}
...
func (m *Manager) RemoveConnectionFromGroup(ctx context.Context, connID, groupID int64) error {
	result, err := m.db.ExecContext(ctx, `
		DELETE FROM cache_group_members WHERE cache_id = ? AND group_id = ?
	`, connID, groupID)
	if err != nil {
		return err
	}
...
func (m *Manager) GetConnectionGroups(ctx context.Context, connID int64) ([]Group, error) {
	rows, err := m.db.QueryContext(ctx, `
		...
		FROM cache_groups g
		JOIN cache_group_members gm ON g.id = gm.group_id
		WHERE gm.cache_id = ?
		ORDER BY g.name
	`, connID)
	if err != nil {
...
func (m *Manager) AddTagToConnection(ctx context.Context, connID, tagID int64) error {
	_, err := m.db.ExecContext(ctx, `
		INSERT OR IGNORE INTO cache_tags (cache_id, tag_id) VALUES (?, ?)
	`, connID, tagID)
	return err
}
...
func (m *Manager) RemoveTagFromConnection(ctx context.Context, connID, tagID int64) error {
	result, err := m.db.ExecContext(ctx, `
		DELETE FROM cache_tags WHERE cache_id = ? AND tag_id = ?
	`, connID, tagID)
	if err != nil {
		return err
	}
...
func (m *Manager) GetConnectionTags(ctx context.Context, connID int64) ([]Tag, error) {
	rows, err := m.db.QueryContext(ctx, `
		...
		FROM tags t
		JOIN cache_tags ct ON t.id = ct.tag_id
		WHERE ct.cache_id = ?
		ORDER BY t.name
	`, connID)
	if err != nil {
