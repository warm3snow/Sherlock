...
3. optional: alias, group, description if provided

Key indicators for each type:
- SSH: keywords like "ssh", "connect", "server", "主机", "服务器", "登录服务器", "连接服务器", common SSH ports (22, 2222)
...
- Port 3306, 33060, 30505, 32630: Usually MySQL database
...
Alias and Group indicators:
- 别名/alias: keywords like "别名", "alias", "名称", "叫做", "命名为"
- 分组/group: keywords like "分组", "group", "组", "环境", "归类到"
- 描述/description: keywords like "描述", "description", "备注", "说明"
...
- "登录服务器 192.168.1.1 用户admin" -> {"type": "ssh", "host": "192.168.1.1", "port": 22, "user": "admin"}
- "连接主机 地址：192.168.1.1:2222, 用户：root" -> {"type": "ssh", "host": "192.168.1.1", "port": 2222, "user": "root"}
- "登录root@192.168.1.1 22 密码是password 别名是test 分组是测试环境" -> {"type": "ssh", "host": "192.168.1.1", "port": 22, "user": "root", "password": "password", "alias": "test", "group": "测试环境"}
- "主机 admin@192.168.1.1 别名webserver 分组生产环境" -> {"type": "ssh", "host": "192.168.1.1", "port": 22, "user": "admin", "alias": "webserver", "group": "生产环境"}

Database connections:
- "mysql -h 192.168.1.1 -P3306 -uroot -ppassword" -> {"type": "database", "host": "192.168.1.1", "port": 3306, "user": "root", "password": "password"}
- "登录数据库 192.168.1.1:3306 用户root 密码root123 别名testdb 分组测试环境" -> {"type": "database", "host": "192.168.1.1", "port": 3306, "user": "root", "password": "root123", "alias": "testdb", "group": "测试环境"}
- "数据库 地址192.168.1.1 端口32630 用户root 密码root123" -> {"type": "database", "host": "192.168.1.1", "port": 32630, "user": "root", "password": "root123"}

Cache connections:
- "redis-cli -h 192.168.1.1 -p 6379" -> {"type": "cache", "host": "192.168.1.1", "port": 6379}
- "连接 redis 192.168.1.1:6379 别名cache1 分组测试" -> {"type": "cache", "host": "192.168.1.1", "port": 6379, "alias": "cache1", "group": "测试"}
- "缓存 地址：192.168.1.1:16379 密码：redis123" -> {"type": "cache", "host": "192.168.1.1", "port": 16379, "password": "redis123"}`
...
// Always uses LLM parsing for better accuracy with natural language inputs.
func (a *Agent) ParseSmartConnectionRequest(ctx context.Context, request string) (*SmartConnectionInfo, error) {
	// Always use AI parsing for all connection types to handle natural language properly
	messages := []*schema.Message{
...
1. host - Manage SSH hosts (list, add, delete, show, edit, update, group, tag)
2. db - Manage MySQL database connections (list, add, delete, show, connect, exec, update, group, tag)
3. cache - Manage Redis cache connections (list, add, delete, show, connect, update, group, tag)
4. connect <id|alias> - Connect to a saved host by ID or alias
...
2. action: The specific action (list, add, delete, show, connect, exec, edit, update, group, tag)
3. args: Additional arguments as an array

IMPORTANT: The "update" action is used to modify alias, group, or description of existing records.
Format for update: args should be [identifier, field, value] or [identifier, "alias", aliasValue, "group", groupValue]
...
- "connect to testdb" -> {"command": "db", "action": "connect", "args": ["testdb"]}
...
- "连接数据库 testdb" -> {"command": "db", "action": "connect", "args": ["testdb"]}
- "查看缓存连接" -> {"command": "cache", "action": "list", "args": []}
- "检查主机连通性" -> {"command": "check", "action": "", "args": []}

Update/Edit examples:
- "给 host 6 增加别名是 test245 分组是 测试环境" -> {"command": "host", "action": "update", "args": ["6", "alias", "test245", "group", "测试环境"]}
- "host 6 别名 test245 分组 测试环境" -> {"command": "host", "action": "update", "args": ["6", "alias", "test245", "group", "测试环境"]}
- "修改主机 6 的别名为 webserver" -> {"command": "host", "action": "update", "args": ["6", "alias", "webserver"]}
- "把数据库 2 的分组改为 生产环境" -> {"command": "db", "action": "update", "args": ["2", "group", "生产环境"]}
- "设置缓存 1 别名 redis-master" -> {"command": "cache", "action": "update", "args": ["1", "alias", "redis-master"]}
- "db 3 别名 mysql-prod 分组 生产环境" -> {"command": "db", "action": "update", "args": ["3", "alias", "mysql-prod", "group", "生产环境"]}
- "更新主机 test245 的分组为 开发环境" -> {"command": "host", "action": "update", "args": ["test245", "group", "开发环境"]}
- "给 cache redis-master 增加分组 生产环境" -> {"command": "cache", "action": "update", "args": ["redis-master", "group", "生产环境"]}
